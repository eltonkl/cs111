...
bool validate_option(option_t* opt)
{
	...
	if(opt->type == file_flag_ && num_args != 0) return false;										//All flags have no args
	if(opt->type == file_open_)																		//All opens have one arg, except pipe which has no args
	{
		if((option == rdwr_ || option == rdonly_ || option == wronly_) && num_args != 1) return false;
		if(option == pipe_ && num_args != 0) return false;
	}
	
	if(opt->type == subcommand_)																	//command has i e o cmd and 0 or more args, wait has no args
	{
		if(option == command_ && num_args < 4) return false;
		if(option == wait_ && num_args != 0) return false;
	}
	
	if(opt->type == misc_)																			//Some misc have no args, some have one arg
	{
		if ((option == catch_ || option == ignore_ || option == default_) && num_args != 1) return false;
		if ((option == verbose_ || option == profile_ || option == abort_ || option == pause_) && num_args != 0) return false;
	}
