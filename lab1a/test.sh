...
./simpsh --rdonly cantpossiblyexist 2>&1 | grep "Failed to open read" > /dev/null
should_succeed "reports missing file";


./simpsh --rdonly Makefile | grep "Failed to open read" > /dev/null;
should_fail "does not report file that exists"


./simpsh --verbose --command 1 2 3 echo foo 2>&1 | grep "not a valid" > /dev/null
...
./simpsh --verbose --command 1 2 3 2>&1 | grep "invalid number of args" > /dev/null
