...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_EXIT:
		...
		for (i = 1; i < NPROCS; i++)
		{
			// If the process has already been waited on before,
			// reap it
			if (proc_array[i].p_state == P_BLOCKED
			    && proc_array[i].p_wait_on == current->p_pid)
			{
				proc_array[i].p_state = P_RUNNABLE;
				proc_array[i].p_registers.reg_eax =
					current->p_exit_status;
	...
	case INT_SYS_NEWTHREAD: {
		...
		current->p_registers.reg_eax = do_newthread(start_function);
		schedule();
	}
	...
	case INT_SYS_KILL: {
		...
		else if (proc_array[p].p_state != P_ZOMBIE)
		{
			...
			//Reap process if it's already been waited on
			int i;
			for (i = 1; i < NPROCS; i++)
			{
				if (proc_array[i].p_state == P_BLOCKED
				    && proc_array[i].p_wait_on == p)
				{
					proc_array[i].p_state = P_RUNNABLE;
...
static pid_t do_newthread(void (*start_function)(void))
{
	...
	process_t* thread = &proc_array[index];
	thread->p_registers = current->p_registers;
	thread->p_registers.reg_esp = PROC1_STACK_ADDR + (thread->p_pid * PROC_STACK_SIZE);
	thread->p_registers.reg_eip = (uint32_t)start_function;
	thread->p_state = P_RUNNABLE;
	return index;
}
...
do_fork(process_t *parent)
{
	...
	if (index == NPROCS)
		return -1;
	process_t* child = &proc_array[index];
