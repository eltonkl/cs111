...
typedef struct process {
	...
	int p_exit_status;		// Process's exit status
        ...
        uint8_t     p_priority;         // 4a. priority level
        bool_t      p_has_run;          // 4a. used to make sure all processes
                                        // of same priority are run equally
        uint8_t     p_share;            // 4b. proportional-share level
        uint32_t    p_used_shares;      // 4b. num shares used
        uint32_t    p_tickets;          // 7. num tickets
} process_t;
